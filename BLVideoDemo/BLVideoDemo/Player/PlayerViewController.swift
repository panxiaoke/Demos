...
fileprivate extension PlayerViewController {
    ...
    func addListeners() {
        ...
        timeObserverToken = player?.addPeriodicTimeObserver(forInterval: interval, queue: DispatchQueue.main, using: {[weak self] (time) in
            ...
            self?.playedDurationLabel.text = self?.formateVideoDuration(duration: Int(current))
            if let durtion = self?.player?.currentItem?.duration {
                let total = CMTimeGetSeconds(durtion)
                if  0 == self?.videoDuration && total <= Double(Int.max){
                    self?.videoDuration = Int(total)
                    self?.progressSlider.maximumValue = Float(total)
                }
            }
